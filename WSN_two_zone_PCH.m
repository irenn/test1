%% Improved WSN Energy Protocol with Two-Zone PCH
% This protocol divides the network into two zones (y<50 and y>50)
...
zone_boundary = 50;         % Y-coordinate boundary between zones
...
%% Initialize Nodes
zone1_nodes = [];           % Nodes with y < 50
zone2_nodes = [];           % Nodes with y >= 50
...
%% Select PCH for Zone 1 (y < 50) - Node closest to zone center
zone1_center_x = xm/2;
zone1_center_y = zone_boundary/2;
min_dist_z1 = inf;
...
%% Select PCH for Zone 2 (y >= 50) - Node closest to zone center
zone2_center_x = xm/2;
zone2_center_y = zone_boundary + (ym - zone_boundary)/2;
min_dist_z2 = inf;
...
fprintf('Zone 1 PCH located at: (%.2f, %.2f)\n', S(PCH1_id).xd, S(PCH1_id).yd);
fprintf('Zone 2 PCH located at: (%.2f, %.2f)\n', S(PCH2_id).xd, S(PCH2_id).yd);
...
for r = 1:rmax
    ...
    % Zone 1 nodes send data to PCH1
    for i = zone1_nodes
        if S(i).E > 0 && i ~= PCH1_id
            % Calculate distance to Zone 1 PCH
    ...
    % Zone 2 nodes send data to PCH2
    for i = zone2_nodes
        if S(i).E > 0 && i ~= PCH2_id
            % Calculate distance to Zone 2 PCH
...
%% Save Results
save('two_zone_results.mat', 'STATISTICS', 'S', 'n', 'rmax', 'first_dead', ...
    'zone1_nodes', 'zone2_nodes', 'PCH1_id', 'PCH2_id');

%% Visualization
figure('Position', [100, 100, 1200, 800]);

% Plot 1: Network Topology with Zones
subplot(2, 2, 1);
hold on;
...
text(5, zone_boundary-5, 'Zone 1 (y < 50)', 'FontSize', 10, 'FontWeight', 'bold');
text(5, zone_boundary+5, 'Zone 2 (y > 50)', 'FontSize', 10, 'FontWeight', 'bold');

xlabel('X (m)');
ylabel('Y (m)');
title('Network Topology - Two-Zone PCH');
...
% Plot 4: Zone Energy Distribution
subplot(2, 2, 4);
...
sgtitle('Two-Zone PCH WSN Protocol');

fprintf('\n=== Two-Zone PCH Protocol Summary ===\n');
...
fprintf('Zone 1 nodes: %d\n', length(zone1_nodes));
fprintf('Zone 2 nodes: %d\n', length(zone2_nodes));
fprintf('====================================\n\n');
