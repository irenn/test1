%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                      %
%          IMPROVED: Two-Zone PCH Protocol (X-BASED ZONES)            %
%          Zone 1 (x < 50) has PCH1, Zone 2 (x >= 50) has PCH2       %
...
%*** NEW: Zone boundary based on X-coordinate ***
zone_boundary = 50;  % x-coordinate dividing zones (LEFT vs RIGHT)
...
for i=1:1:n
    ...
    %*** NEW: Assign zone based on X-coordinate ***
    if S(i).xd < zone_boundary
        S(i).zone = 1;  % Left zone
    else
        S(i).zone = 2;  % Right zone
    end
   ...
   temp_rnd0=i;
    if (temp_rnd0 >= 0 && temp_rnd0 < 50 )
        ...
        S(i).E=t*Eo;
        if S(i).zone == 1
            plot(S(i).xd,S(i).yd,'bo');  % Blue circle for zone 1 (left)
        else
            plot(S(i).xd,S(i).yd,'ro');  % Red circle for zone 2 (right)
        end
        hold on;
    end
    if (temp_rnd0 >= 50 && temp_rnd0 < 100 )
        ...
        S(i).E=t*Eo;
        if S(i).zone == 1
            plot(S(i).xd,S(i).yd,'b+');  % Blue + for zone 1 (left)
        else
            plot(S(i).xd,S(i).yd,'r+');  % Red + for zone 2 (right)
        end
        hold on;
    end
...
% Draw zone boundary (vertical line at x=50)
plot([zone_boundary zone_boundary], [0 ym], 'k--', 'LineWidth', 2);
plot(S(n+1).xd,S(n+1).yd,'mx', 'MarkerSize', 15, 'LineWidth', 3);
text(zone_boundary-15, ym-5, 'Zone 1 (x < 50)', 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'blue');
text(zone_boundary+5, ym-5, 'Zone 2 (x >= 50)', 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'red');
title('Two-Zone PCH: Left (x<50) vs Right (x>=50)');
...
dead=0;
dead_z1=0;  % Dead in zone 1 (left)
dead_z2=0;  % Dead in zone 2 (right)
...
for i=1:1:n
    ...
    if (S(i).E<=0)
        plot(S(i).xd,S(i).yd,'kx', 'MarkerSize', 8);
        dead=dead+1;
        if S(i).zone == 1
            dead_z1 = dead_z1 + 1;
        else
    ...
    if S(i).E>0
        ...
        if S(i).zone == 1
            ...
            plot(S(i).xd,S(i).yd,'ro', 'MarkerSize', 4);
...
% Draw zone boundary (vertical line)
plot([zone_boundary zone_boundary], [0 ym], 'k--', 'LineWidth', 2);
plot(S(n+1).xd,S(n+1).yd,'mx', 'MarkerSize', 15, 'LineWidth', 3);
...
for i=1:1:n
 ...
 if(temp_rand<= (S(i).p/(1-S(i).p*mod(r,round(1/S(i).p)))))
            ...
            if S(i).zone == 1
                plot(S(i).xd,S(i).yd,'b*', 'MarkerSize', 10);  % Blue star for Zone 1 CH (left)
            else
                plot(S(i).xd,S(i).yd,'r*', 'MarkerSize', 10);  % Red star for Zone 2 CH (right)
            end
...
%%%*** Based on X-coordinate zones ***%%%

    % PCH1 for Zone 1 (x < 50) - LEFT ZONE
    trouve_z1=true;
    pch1=0;
    for i=1:1:n
    ...
    % PCH2 for Zone 2 (x >= 50) - RIGHT ZONE
    trouve_z2=true;
    pch2=0;
    for i=1:1:n
    ...
    if pch1 ~= 0
        ...
        text(S(pch1).xd, S(pch1).yd+5, 'PCH1', 'FontSize', 9, 'Color', 'green', 'FontWeight', 'bold');
    end

    % Mark PCH2 (Right Zone)
    if pch2 ~= 0
        S(pch2).type='P';
        P2.xd=S(pch2).xd;
        P2.yd=S(pch2).yd;
        ...
        text(S(pch2).xd, S(pch2).yd+5, 'PCH2', 'FontSize', 9, 'Color', 'magenta', 'FontWeight', 'bold');
    end
    ...
    for i=1:1:n
        ...
        if(S(i).type=='C' && S(i).E>0)
            ...
            % Zone 1 CHs (LEFT) route to PCH1 or BS
            ...
            % Zone 2 CHs (RIGHT) route to PCH2 or BS
    ...
    if pch1 ~= 0 && S(pch1).E > 0
        ...
        plot([S(pch1).xd S(n+1).xd],[S(pch1).yd S(n+1).yd],'g-.', 'LineWidth', 1.5);
    end
    ...
    if pch2 ~= 0 && S(pch2).E > 0
        ...
        plot([S(pch2).xd S(n+1).xd],[S(pch2).yd S(n+1).yd],'m-.', 'LineWidth', 1.5);
    end
     ...
     if(cluster-1>=1)
       ...
       for c=1:1:cluster-1
        ...
        if(min_dis>0)
            ...
            if S(i).zone == 1
                plot([S(i).xd S(C(min_dis_cluster).id).xd],[S(i).yd S(C(min_dis_cluster).id).yd],'b-', 'LineWidth', 0.3);
            else
                plot([S(i).xd S(C(min_dis_cluster).id).xd],[S(i).yd S(C(min_dis_cluster).id).yd],'r-', 'LineWidth', 0.3);
            end
        end
       S(i).min_dis=min_dis;
...
title('IMPROVED: Two-Zone PCH X-based (x<50 LEFT, x>=50 RIGHT)');

%Packets received at BS
figure(3);
...
title('IMPROVED: Two-Zone PCH X-based (a=5 and m=0.1)');
...
title('IMPROVED: Two-Zone PCH X-based');
...
% NEW: Zone energy distribution over time (LEFT vs RIGHT)
figure(8);
...
hold on;
plot(rounds_vec, zone2_energy_vec, 'r-', 'LineWidth', 2);
plot(rounds_vec, zone1_energy_vec + zone2_energy_vec, 'k--', 'LineWidth', 1.5);
xlabel('Rounds');
ylabel('Energy (J)');
title('Energy Distribution: LEFT Zone (x<50) vs RIGHT Zone (x>=50)');
legend('Left Zone (x < 50)', 'Right Zone (x >= 50)', 'Total', 'Location', 'best');
grid on;

% Save results
save('improved_xbased_twozone_pch_results.mat', 'STATISTICS', 'CC', 'PBS', 'MKK', 'RR', 'PRBS', 'FDD', 'first_dead', 'Etot');
